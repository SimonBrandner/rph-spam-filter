...
class BinaryConfusionMatrixTest(unittest.TestCase):
    ...
    def test_computeFromDicts_allCasesOnce(self):
        # Prepare fixture
        # The items of truth and prediction dicts are intentionally in different order
        # to detect errors when someone assumes that the keys are in the same order
        truth = {1: SPAM_TAG, 2: SPAM_TAG, 3: HAM_TAG, 4: HAM_TAG}
        pred = {1: SPAM_TAG, 3: SPAM_TAG, 4: HAM_TAG, 2: HAM_TAG}
        # Excercise the SUT
        self.cm.compute_from_dicts(truth, pred)
        # Assert
